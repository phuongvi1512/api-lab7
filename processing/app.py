...
kafka_hostname = f"{app_config['events']['hostname']}:
...
def populate_stats():
    ...
    if report_body.status_code != 200 or cfile_body.status_code != 200:
        ...
        try:
            stats_record = StatsFile(num_reports=num_reports,
                                     num_files=num_files,
                                     max_temp=max_temp,
            ...
            logger.error(f"failed to insert updated stats to the sqlite database at {datetime.now()}. error is {e}")
